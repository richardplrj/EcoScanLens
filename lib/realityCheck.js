...
function mapFromItemNameAndBlob(blob) {
  ...
  const rules = [
    ...
    [/\blay'?s\b|chip packet|crisp packet|snack wrapper|biscuit wrapper|candy wrapper|instant noodle pack|coffee sachet|laminated pouch|multi-layer|multilayer|blister pack|shrink wrap snack/, "Multi_Layer"],
...
function isValidMaterialKey(v) {
  return MATERIAL_KEYS.includes(String(v ?? ""));
}

export function resolveMaterialKeyFromResult(result) {
  ...
  const materialKeyGpt = String(result?.material_key ?? "");
  ...
  let material =
    (isValidMaterialKey(materialKeyGpt) ? materialKeyGpt : null) ||
...
export function getRealityCheckFromGptResult(result) {
  ...
  const material = resolveMaterialKeyFromResult(result);
  const recyclabilityReason = String(result.recyclability_reason ?? "");
  const sortingExplanation = String(result.sorting_explanation ?? "");
  const disposalGuidance = String(result.disposal_guidance ?? "");
  const blob = [result.item_name, result.material_type, recyclabilityReason, sortingExplanation, disposalGuidance].join(
    " "
  );
